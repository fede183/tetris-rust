use bevy::prelude::*;
use crate::config::*;
use crate::common::{draw_rectable_with_border, draw_rectable};
use rand::Rng;
...
#[derive(Component)]
pub struct Piece(Vec<PointOnBoard>);

#[derive(Component)]
pub struct NextPiece(Piece);
...
pub fn setup_next_piece_board(mut commands: Commands, query: Query<&NextPiece>) {
    ...
    for points in &query {
        for point in &points.0.0 {
            ...
            draw_rectable(&mut commands, Vec3{ x: DISPLAY_NEXT_PIECE_POSITION_X + x_position, y: DISPLAY_NEXT_PIECE_POSITION_Y + y_position, z: 1.}, SQUARE_SIZE, SQUARE_SIZE, point.color);
        }
    }
}
